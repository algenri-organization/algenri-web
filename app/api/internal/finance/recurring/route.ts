...
import { listRecurringSeries, setRecurringSeriesPaused } from "@/lib/finance/recurring-store";
...
export async function PATCH(request: Request) {
  try {
    ...
    if (!seriesId || typeof input.paused !== "boolean") return Response.json({ ok: false, error: "invalid_request" }, { status: 400 });
    ...
    const status = ["series_not_found"].includes(code) ? 404 : ["no_future_occurrences"].includes(code) ? 400 : 500;
