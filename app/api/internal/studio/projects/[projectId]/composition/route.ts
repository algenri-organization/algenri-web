...
const brandSchema = z.object({
  mode: z.enum(["algenri", "asset", "text", "none"]),
  name: z.string().max(80),
  logoStoragePath: z.string().max(500).nullable().optional(),
  logoContentType: z.string().max(120).nullable().optional(),
});
...
async function authorize(request: Request, projectId: string): Promise<AuthorizationResult> {
  ...
  if (!project) return { ok: false, response: Response.json({ ok: false, error: "not_found" }, { status: 404 }) };
  if (project.ownerUid && project.ownerUid !== user.uid) return { ok: false, response: Response.json({ ok: false, error: "forbidden" }, { status: 403 }) };
