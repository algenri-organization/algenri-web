...
const briefingSchema = z.object({
  ...
  scriptMode: z.enum(["ai", "manual"]), script: z.string().trim().max(6000).optional(), visualStyle: z.string().trim().min(2).max(200), aspectRatio: z.enum(["16:9", "9:16", "1:1", "4:5"]), useBrandIdentity: z.boolean(), useAvatar: z.boolean(), avatarId: z.string().trim().max(200).optional(), useVoice: z.boolean(), voiceId: z.string().trim().max(200).optional(), pronunciationNotes: z.string().trim().max(1500).optional(), requiredScenes: z.string().trim().max(3000).optional(), requiredOnScreenText: z.string().trim().max(2000).optional(), prohibitedElements: z.string().trim().max(3000).optional(), referenceNotes: z.string().trim().max(3000).optional(), priority: z.enum(["quality", "cost", "speed", "balanced"]), engineMode: z.enum(["automatic", "manual"]), manualEngineId: z.string().trim().max(120).optional(), budgetLimit: z.number().min(0).max(1000000).optional(),
});
