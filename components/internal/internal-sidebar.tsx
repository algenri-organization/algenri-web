...
const groups: Group[] = [
  { label: "Comercial", icon: BriefcaseBusiness, items: [
  ...
  { label: "Operação", icon: Gauge, items: [
  ...
  { label: "Financeiro", icon: CircleDollarSign, items: [
...
export default function InternalSidebar() {
  ...
  const [collapsed, setCollapsed] = useState(false); const [mobileOpen, setMobileOpen] = useState(false); const [user, setUser] = useState<User | null>(firebaseAuth.currentUser); const [overdueCount,setOverdueCount]=useState(0);
  ...
  useEffect(()=>{ if(!user){setOverdueCount(0);return;} let cancelled=false; (async()=>{ try{ const token=await user.getIdToken(); const r=await fetch("/api/internal/finance/charges",{headers:{Authorization:`Bearer ${token}`}}); if(!r.ok)return; const p=await r.json(); const today=new Date().toISOString().slice(0,10); const count=(p.charges??[]).filter((c:ChargeSummary)=>c.status==="pending"&&c.dueDate<today).length; if(!cancelled)setOverdueCount(count); }catch{} })(); return()=>{cancelled=true}; },[user,pathname]);
