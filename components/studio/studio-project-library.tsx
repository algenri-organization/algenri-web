...
import { ArrowRight, Clapperboard, Clock3, FolderOpen, Loader2, Plus, RefreshCw, TriangleAlert } from "lucide-react";
...
export default function StudioProjectLibrary() {
  ...
  return <>
    <section className="mt-7 rounded-[26px] border border-white/10 bg-white/[.02] p-5 sm:p-6">
      <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        <div>
          ...
          <p className="mt-2 text-sm leading-6 text-white/45">Os projetos criados ficam persistidos e podem ser reabertos com storyboard, versões, composição e render final.</p>
        </div>
    ...
    {!loading && !error && hasProjects && <section className="mt-5 grid gap-4 md:grid-cols-2 xl:grid-cols-3">
      {projects.map((project) => <a key={project.id} href={`/interno/lab/studio/projetos/${project.id}`} className="group rounded-[24px] border border-white/10 bg-white/[.025] p-5 transition hover:border-cyan-300/25 hover:bg-cyan-300/[.035]">
        ...
        <div className="mt-5 flex items-center justify-between border-t border-white/10 pt-4 text-xs text-white/35"><span className="inline-flex items-center gap-1.5"><Clock3 className="h-3.5 w-3.5"/>{formatDate(project.updatedAt || project.createdAt)}</span><span className="inline-flex items-center gap-1 font-semibold text-cyan-100 transition group-hover:translate-x-0.5">Abrir <ArrowRight className="h-3.5 w-3.5"/></span></div>
      </a>)}
    </section>}
