...
import { Bot, CheckCircle2, Coins, Loader2, Route, ShieldCheck, TriangleAlert } from "lucide-react";
...
export default function StudioRoutingPanel({projectId,allApproved,initialRouting}:{projectId:string;allApproved:boolean;initialRouting?:RoutingPlan|null}){
  ...
  return <section className="mt-8 rounded-[28px] border border-cyan-300/15 bg-cyan-300/[.025] p-5 sm:p-6">
    <div className="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between"><div><div className="flex items-center gap-2 text-cyan-100"><Route className="h-5 w-5"/><h2 className="text-lg font-semibold">Roteamento de motores & custo</h2></div><p className="mt-2 max-w-3xl text-sm leading-6 text-white/45">Com o storyboard aprovado, o Studio compara as rotas ativas, valida o Runway por dry run e consulta o saldo da Kie.ai antes de qualquer geração.</p>{routing?.policyVersion&&<p className="mt-2 text-[11px] text-white/25">Política de decisão: {routing.policyVersion}</p>}</div><div className="flex flex-wrap gap-2"><a href={`/interno/lab/studio/projetos/${projectId}/governanca`} className="inline-flex items-center justify-center gap-2 rounded-xl border border-white/10 px-4 py-3 text-sm font-semibold text-white/60"><ShieldCheck className="h-4 w-4"/>Governança</a><button onClick={calculate} disabled={!allApproved||loading} className="inline-flex items-center justify-center gap-2 rounded-xl border border-cyan-300/20 bg-cyan-300/[.08] px-4 py-3 text-sm font-semibold text-cyan-100 disabled:cursor-not-allowed disabled:opacity-35">{loading?<Loader2 className="h-4 w-4 animate-spin"/>:<Coins className="h-4 w-4"/>}{routing?"Recalcular rota":"Calcular motores e custo"}</button></div></div>
