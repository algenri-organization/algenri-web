...
import { getPublishedContractModel } from "@/lib/contracts/document-models";
...
export type ContractRecord={
  ...
  sourceDocumentModelId:string|null;sourceDocumentModelName:string|null;sourceDocumentModelVersion:number|null;documentModelContent:string;
...
export async function createContract(input:{proposalId:string;title?:string;startDate?:string;endDate?:string;notes?:string;modelId?:string},createdBy:string){
  ...
  const model=input.modelId?await getPublishedContractModel(input.modelId):null;
  ...
  await db.runTransaction(async tx=>{const c=await tx.get(counterRef);const next=(c.data()?.value??0)+1;record={id:ref.id,tenantId:TENANT,contractNumber:`CONT-${year}-${String(next).padStart(4,"0")}`,clientId:proposal.clientId,clientName:proposal.clientName,projectId:proposal.projectId,projectName:proposal.projectName,proposalId:proposal.id,proposalNumber:proposal.proposalNumber,proposalVersion:proposal.version,status:"draft",title:text(input.title)||`Contrato — ${proposal.projectName}`,startDate:text(input.startDate),endDate:text(input.endDate),notes:text(input.notes),initialValue:proposal.total,recurringMonthly:proposal.recurringMonthly,recurringAnnual:proposal.recurringAnnual,paymentTerms:proposal.paymentTerms,sourceDocumentModelId:model?.id??null,sourceDocumentModelName:model?.name??null,sourceDocumentModelVersion:model?.version??null,documentModelContent:model?.content??"",signedFileName:null,signedFilePath:null,signedFileUploadedAt:null,signedFileUploadedBy:null,createdAt:iso,createdBy,updatedAt:iso,updatedBy:createdBy,signedAt:null,cancelledAt:null};tx.set(counterRef,{value:next,updatedAt:iso},{merge:true});tx.set(ref,record);});
  if(project.status!=="contract")await updateProject(project.id,{status:"contract"});
  return record;
}
export async function updateContract(id:string,input:Record<string,unknown>,updatedBy:string){const current=await getContract(id);if(!current)return null;if(current.status==="signed"||current.status==="cancelled")throw new Error("contract_locked");const now=new Date().toISOString();const patch={title:text(input.title)||current.title,startDate:text(input.startDate),endDate:text(input.endDate),notes:text(input.notes),documentModelContent:input.documentModelContent===undefined?(current.documentModelContent??""):text(input.documentModelContent),updatedAt:now,updatedBy};const db=await getAdminDb();await db.collection(CONTRACTS).doc(id).set(patch,{merge:true});return getContract(id);}
