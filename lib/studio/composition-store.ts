...
function defaultsForProject(project: any, brand = brandForProject(project)): StudioSceneOverlay[] {
  ...
  return scenes.map((scene: any, index: number) => ({
    ...
    align: "left" as const, position: index === scenes.length - 1 ? "bottom" as const : "center" as const,
...
export async function getStudioComposition(projectId: string): Promise<StudioComposition> {
  ...
  if (stored?.sceneOverlays?.length) return { ...stored, brand: stored.brand ?? fallbackBrand, preset: stored.preset ?? "editorial" };
...
export async function saveStudioComposition(projectId: string, input: { brand?: StudioBrandConfig; preset?: StudioCompositionPreset; sceneOverlays: StudioSceneOverlay[]; transition: "cut" | "fade"; approve?: boolean }) {
  ...
  const overlays = input.sceneOverlays.filter((item) => storyboardIndices.has(Number(item.sceneIndex))).map((item): StudioSceneOverlay => ({
    sceneIndex: Number(item.sceneIndex), enabled: Boolean(item.enabled), eyebrow: String(item.eyebrow ?? "").slice(0, 80), headline: String(item.headline ?? "").slice(0, 160), body: String(item.body ?? "").slice(0, 280), cta: String(item.cta ?? "").slice(0, 100),
    align: item.align === "center" || item.align === "right" ? item.align : "left", position: item.position === "top" || item.position === "bottom" ? item.position : "center", showBrand: brand.mode !== "none" && Boolean(item.showBrand),
  }));
  const now = new Date().toISOString();
  const composition: StudioComposition = { state: input.approve ? "approved" : "draft", brand, preset: input.preset === "commercial" || input.preset === "minimal" ? input.preset : "editorial", sceneOverlays: overlays, transition: input.transition === "cut" ? "cut" : "fade", approvedAt: input.approve ? now : null, updatedAt: now };
