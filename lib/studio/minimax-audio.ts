...
export async function generateMiniMaxSpeech(input:{text:string;voiceId:string;model?:string;speed?:number;volume?:number;pitch?:number;emotion?:string;format?:"mp3"|"wav"|"flac"|"pcm"}){
 const response=await fetch(`${BASE_URL}/v1/t2a_v2`,{
  ...
  body:JSON.stringify({model:input.model||MINIMAX_DEFAULT_MODEL,text:input.text,stream:false,voice_setting:{voice_id:input.voiceId,speed:input.speed??1,vol:input.volume??1,pitch:input.pitch??0,...(input.emotion?{emotion:input.emotion}:{})},audio_setting:{format:input.format||"mp3",sample_rate:32000,bitrate:128000,channel:1}}),
 });
 ...
 const audio=payload?.data?.audio??payload?.audio_file??payload?.audio;
