...
function continuityMode(briefing: any) {
...
export async function generateStudioStoryboardWithAi(projectId: string) {
  ...
  const context = { studioProject: { name: project.name, origin: project.commercialLink?.origin ?? "internal" }, briefing, commercial: await commercialContext(project), existingContinuity: project.continuity ?? null };
  ...
  try {
    ...
    await saveStudioContinuity(projectId, { mode: continuityMode(briefing), ...result.continuityBible });
    return { storyboard: scenes, continuity: result.continuityBible, model };
  } catch (error) {
...
export async function regenerateStudioSceneWithAi(projectId: string, sceneIndex: number, instructions?: string) {
  ...
  const context = { project: { name: project.name, briefing: project.briefing, commercial: await commercialContext(project) }, continuityBible: project.continuity ?? null, previousScene: previous, currentScene: current, nextScene: next, userInstructions: instructions?.trim() || "Melhore a cena sem quebrar a continuidade do vídeo." };
