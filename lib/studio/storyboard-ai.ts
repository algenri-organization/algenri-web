...
export async function regenerateStudioSceneWithAi(projectId: string, sceneIndex: number, instructions?: string) {
  ...
  const prompt = `Você é o diretor criativo do ALGENRI Studio. Reescreva SOMENTE a cena ${sceneIndex}, preservando exatamente ${current.durationSeconds} segundos e a continuidade com as cenas anterior e posterior. Não altere fatos comerciais. Logos, nomes de marca, slogans, CTAs e textos legíveis ficam para composição posterior controlada. O technicalPrompt deve pedir apenas o visual-base da cena e deve proibir logos, texto, marcas d'água e símbolos corporativos inventados. Respeite pronúncias e restrições do briefing.\n\nCONTEXTO:\n${JSON.stringify(context)}`;
