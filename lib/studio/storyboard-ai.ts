...
export async function generateStudioStoryboardWithAi(projectId: string) {
  ...
  const context = { studioProject: { name: project.name, origin: project.commercialLink?.origin ?? "internal" }, briefing, commercial: await commercialContext(project), existingContinuity };
